...
import { DashboardFilterDTO, DashboardMetricDTO } from "@/types/dashboardTypes";
import { useEffect, useState } from "react";

export function useDashboardCountData() {
    const [timeInterval, setTimeInterval] = useState<FilterTimeIntervals>(FilterTimeIntervals.MONTHLY);
    const [dateRange, setDateRange] = useState<[Date | null, Date | null] | null>(null);
    const [isLoading, setIsLoading] = useState(false);
    const [salesLineChartData, setSalesLineChartData] = useState<DashboardMetricDTO>();
    ...
    const fetchGrossExpenseData = async () => {
        ...
        const response = await dashboardService.GetDashboardCounts(payload);
...
export type UseAppointmentChartReturn = ReturnType<typeof useDashboardCountData>;